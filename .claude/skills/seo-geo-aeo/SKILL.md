...
# SEO / GEO / AEO Audit Skill (Claude Code edition)
...
> **Environment note (Claude Code):** This skill was adapted from a claude.ai desktop version. In Claude Code you MUST use `curl` to fetch raw HTML (the built-in WebFetch tool converts pages to Markdown and strips the `<meta>`, `<link rel=canonical>`, Open Graph, and JSON-LD schema tags this audit depends on). The report is written to a local file, not a claude.ai sandbox path. There is no `present_files` or `computer://` — surface the file with a clickable relative path and offer to publish it via the Artifact tool.
...
## Step 5: Generate the downloadable report (Claude Code)
